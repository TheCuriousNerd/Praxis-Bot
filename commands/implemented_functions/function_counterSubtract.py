...
class Function_CounterSubtract(AbstractCommandFunction, metaclass=ABCMeta):
    """
    This is v0 of Functions
    """
    functionName = "subtractCounter"
    warningText = []
    helpText = ["This subtracts to a counter variable.",
        "\nExample:","($subtractCounter (var)(data))"]

    def __init__(self):
        super().__init__(
            functionName = Function_CounterSubtract.functionName,
            n_args = 0,
            functionType = AbstractCommandFunction.FunctionType.ver0,
            helpText = Function_CounterSubtract.helpText,
