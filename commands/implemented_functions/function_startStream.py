...
class Function_startStream(AbstractCommandFunction, metaclass=ABCMeta):
    """
    This is v0 of Functions
    """
    functionName = "startStream"
    warningText = []
    helpText = ["This will tell OBS to start streaming.",
        "\nExample:","($startStream)"]

    def __init__(self):
        super().__init__(
            functionName = Function_startStream.functionName,
            n_args = 0,
            functionType = AbstractCommandFunction.FunctionType.ver0,
            helpText = Function_startStream.helpText,
    ...
    def do_work(self, user, functionName, args, bonusData):
        ...
        try:
            ...
            output:simpleobsws.RequestResponse = obsWebSocket.makeRequest("StartStream", {})
            print(output)
