...
class Function_pauseRecording(AbstractCommandFunction, metaclass=ABCMeta):
    """
    This is v0 of Functions
    """
    functionName = "pauseRecording"
    warningText = []
    helpText = ["This will tell OBS to pause recording.",
        "\nExample:","($pauseRecording)"]

    def __init__(self):
        super().__init__(
            functionName = Function_pauseRecording.functionName,
            n_args = 0,
            functionType = AbstractCommandFunction.FunctionType.ver0,
            helpText = Function_pauseRecording.helpText,
    ...
    def do_work(self, user, functionName, args, bonusData):
        ...
        try:
            ...
            output:simpleobsws.RequestResponse = obsWebSocket.makeRequest("PauseRecord", {})
            print(output)
