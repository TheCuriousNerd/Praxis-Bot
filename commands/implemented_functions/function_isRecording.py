...
class Function_IsRecording(AbstractCommandFunction, metaclass=ABCMeta):
    """
    This is v0 of Functions
    """
    functionName = "isRecording"
    warningText = []
    helpText = ["This will return true if OBS is currently recording.",
        "\nExample:","($isRecording)"]

    def __init__(self):
        super().__init__(
            functionName = Function_IsRecording.functionName,
            n_args = 0,
            functionType = AbstractCommandFunction.FunctionType.ver0,
            helpText = Function_IsRecording.helpText,
    ...
    def do_work(self, user, functionName, args, bonusData):
        ...
        try:
            ...
            output:simpleobsws.RequestResponse = obsWebSocket.makeRequest("GetRecordStatus", {})
            print(output)
