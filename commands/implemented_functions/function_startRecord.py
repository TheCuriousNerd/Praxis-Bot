...
class Function_startRecording(AbstractCommandFunction, metaclass=ABCMeta):
    """
    This is v0 of Functions
    """
    functionName = "startRecording"
    warningText = []
    helpText = ["This will tell OBS to start recording.",
        "\nExample:","($startRecording)"]

    def __init__(self):
        super().__init__(
            functionName = Function_startRecording.functionName,
            n_args = 0,
            functionType = AbstractCommandFunction.FunctionType.ver0,
            helpText = Function_startRecording.helpText,
    ...
    def do_work(self, user, functionName, args, bonusData):
        ...
        try:
            ...
            output:simpleobsws.RequestResponse = obsWebSocket.makeRequest("StartRecord", {})
            print(output)
