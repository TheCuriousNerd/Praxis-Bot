...
class Function_discordVoice_join(AbstractCommandFunction, metaclass=ABCMeta):
    """
    This is v0 of Functions
    """
    functionName = "discordVoiceJoin"
    warningText = ["This function is not yet implemented."]
    helpText = ["This will cause the bot to join a voice channel.",
        "\nExample:","($discordVoiceJoin (channel_id))"]

    def __init__(self):
        super().__init__(
            functionName = Function_discordVoice_join.functionName,
            n_args = 0,
            functionType = AbstractCommandFunction.FunctionType.ver0,
            helpText = Function_discordVoice_join.helpText,
    ...
    def do_work(self, user, functionName, args, bonusData):
        ...
        db.add_taskToQueue("standalone_discord", "voice", str(time.time()), "join", args[0], user)
        return args
