...
class Function_discordVoice_repeat(AbstractCommandFunction, metaclass=ABCMeta):
    """
    This is v0 of Functions
    """
    functionName = "discordVoiceRepeat"
    warningText = ["This function is not yet implemented."]
    helpText = ["This will cause the bot to repeat the current voice queue.",
        "\nExample:","($discordVoiceRepeat (true))"]

    def __init__(self):
        super().__init__(
            functionName = Function_discordVoice_repeat.functionName,
            n_args = 0,
            functionType = AbstractCommandFunction.FunctionType.ver0,
            helpText = Function_discordVoice_repeat.helpText,
    ...
    def do_work(self, user, functionName, args, bonusData):
        ...
        db.add_taskToQueue("standalone_discord", "voice", str(time.time()), "repeat", arg, "")
        return args
