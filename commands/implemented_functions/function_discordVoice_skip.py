...
class Function_discordVoice_skip(AbstractCommandFunction, metaclass=ABCMeta):
    """
    This is v0 of Functions
    """
    functionName = "discordVoiceSkip"
    warningText = ["This function is not yet implemented."]
    helpText = ["This will cause the bot to skip to the next entry in the queue.",
        "\nExample:","($discordVoiceSkip)"]

    def __init__(self):
        super().__init__(
            functionName = Function_discordVoice_skip.functionName,
            n_args = 0,
            functionType = AbstractCommandFunction.FunctionType.ver0,
            helpText = Function_discordVoice_skip.helpText,
    ...
    def do_work(self, user, functionName, args, bonusData):
        ...
        db.add_taskToQueue("standalone_discord", "voice", str(time.time()), "skip", "", "")
        return args
