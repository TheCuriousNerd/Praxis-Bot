...
class Function_discordVoice_join(AbstractCommandFunction, metaclass=ABCMeta):
    ...
    """
    functionName = "discordVoiceLeave"
    warningText = ["This function is not yet fully implemented."]
    helpText = ["This will cause the bot to leave the voice channel.",
        "\nExample:","($discordVoiceLeave)"]
    ...
    def do_work(self, user, functionName, args, bonusData):
        ...
        db.add_taskToQueue("standalone_discord", "voice", str(time.time()), "leave", "", "")
        return args
