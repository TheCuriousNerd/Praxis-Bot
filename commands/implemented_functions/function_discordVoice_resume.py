...
class Function_discordVoice_resume(AbstractCommandFunction, metaclass=ABCMeta):
    """
    ...
    def __init__(self):
        super().__init__(
            functionName = Function_discordVoice_resume.functionName,
            n_args = 0,
            functionType = AbstractCommandFunction.FunctionType.ver0,
            helpText = Function_discordVoice_resume.helpText,
    ...
    def do_work(self, user, functionName, args, bonusData):
        ...
        db.add_taskToQueue("standalone_discord", "voice", str(time.time()), "resume", "", "")
        return args
