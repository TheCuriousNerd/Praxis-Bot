...
class Function_Range(AbstractCommandFunction, metaclass=ABCMeta):
    """
    ...
    def __init__(self):
        super().__init__(
            functionName = Function_Range.functionName,
            n_args = 0,
            functionType = AbstractCommandFunction.FunctionType.ver0,
            helpText = Function_Range.helpText,
            bonusFunctionData = None
            )

    def do_function(self, tokenSource, user, functionName, args, bonusData):
        output = self.do_work(user, functionName, args, tokenSource)

        return output

    def do_work(self, user, functionName, args, tokenSource):
