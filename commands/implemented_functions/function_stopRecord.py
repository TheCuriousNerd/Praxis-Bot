...
class Function_stopRecording(AbstractCommandFunction, metaclass=ABCMeta):
    """
    This is v0 of Functions
    """
    functionName = "stopRecording"
    warningText = []
    helpText = ["This will tell OBS to stop recording.",
        "\nExample:","($stopRecording)"]

    def __init__(self):
        super().__init__(
            functionName = Function_stopRecording.functionName,
            n_args = 0,
            functionType = AbstractCommandFunction.FunctionType.ver0,
            helpText = Function_stopRecording.helpText,
    ...
    def do_work(self, user, functionName, args, bonusData):
        ...
        try:
            ...
            output:simpleobsws.RequestResponse = obsWebSocket.makeRequest("StopRecord", {})
            print(output)
