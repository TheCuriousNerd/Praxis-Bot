...
class Function_discordVoice_pause(AbstractCommandFunction, metaclass=ABCMeta):
    """
    This is v0 of Functions
    """
    functionName = "discordVoicePause"
    warningText = ["This function is not yet implemented."]
    helpText = ["This will cause the bot to pause it's voice on discord.",
        "\nExample:","($discordVoicePause)"]

    def __init__(self):
        super().__init__(
            functionName = Function_discordVoice_pause.functionName,
            n_args = 0,
            functionType = AbstractCommandFunction.FunctionType.ver0,
            helpText = Function_discordVoice_pause.helpText,
    ...
    def do_work(self, user, functionName, args, bonusData):
        ...
        db.add_taskToQueue("standalone_discord", "voice", str(time.time()), "pause", "", "")
        return args
