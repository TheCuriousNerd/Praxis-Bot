...
class Function_discordVoice_loop(AbstractCommandFunction, metaclass=ABCMeta):
    """
    ...
    def __init__(self):
        super().__init__(
            functionName = Function_discordVoice_loop.functionName,
            n_args = 0,
            functionType = AbstractCommandFunction.FunctionType.ver0,
            helpText = Function_discordVoice_loop.helpText,
    ...
    def do_work(self, user, functionName, args, bonusData):
        ...
        db.add_taskToQueue("standalone_discord", "voice", str(time.time()), "loop", arg, "")
        return args
